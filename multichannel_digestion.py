...
def run(protocol: protocol_api.ProtocolContext):
    ...
    PacI = protocol.define_liquid(name="PacI enzyme", description= "Enzyme for digesion", display_color= "#FF7820")
    SpeI = protocol.define_liquid(name="SpeI enzyme", description= "Enzyme for digesion", display_color= "#84FBB1")
    ...
    for i in range(0,8):
        reagent_plate.columns()[0][i].load_liquid(liquid=PacI, volume = ((enzyme_volume1 + enzyme_volume2)*column_no)/8)
        reagent_plate.columns()[1][i].load_liquid(liquid=SpeI, volume = ((enzyme_volume1 + enzyme_volume2)*column_no)/8)
